...
func TestGetBestProxy(t *testing.T) {
	...
	tests := []struct {
		...
		{"Success", []string{server.URL}, false},
		{"Bad data", []string{server.URL + "/bad"}, true},
		{"Empty response", []string{server.URL + "/empty"}, true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := getBestProxy(tt.urls, 60*time.Second)
